...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $stmt->execute([$username, $email, $password, $phone, $district]);

        // Get the newly inserted user's ID
        $student_id = $pdo->lastInsertId();

        // Insert a record into video_access (without video_id)
        $stmt2 = $pdo->prepare("INSERT INTO video_access (student_id, name, email,district, video_id) VALUES (?, ?, ?,?, NULL)");
        $stmt2->execute([$student_id, $username, $email, $district]);
